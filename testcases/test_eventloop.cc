...
#include "EasyTcp/net/io_thread_group.h"
...
void test_io_thread()
{
  ...
  // EasyTcp::FdEvent event(listenfd);
  // event.listen(EasyTcp::FdEvent::IN_EVENT, [listenfd](){
  ...
  int i = 0;
  EasyTcp::TimerEvent::s_ptr timer_event = std::make_shared<EasyTcp::TimerEvent>(
      1000, true, [&i]()
      { INFOLOG("trigger timer event, count=%d", i++); });

  // EasyTcp::IOThread io_thread;
  ...
  EasyTcp::IOThreadGroup io_thread_group(2);
  EasyTcp::IOThread *io_thread = io_thread_group.getIOThread();
  // io_thread->getEventLoop()->addEpollEvent(&event);
  io_thread->getEventLoop()->addTimerEvent(timer_event);

  EasyTcp::IOThread *io_thread2 = io_thread_group.getIOThread();
...
int main()
{
  ...
  // EasyTcp::EventLoop* eventloop = new EasyTcp::EventLoop();
