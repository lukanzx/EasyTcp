...
#include "EasyTcp/net/rpc/rpc_controller.h"
#include "EasyTcp/net/rpc/rpc_channel.h"
#include "EasyTcp/net/rpc/rpc_closure.h"
...
void test_tcp_client()
{
  ...
  EasyTcp::IPNetAddr::s_ptr addr = std::make_shared<EasyTcp::IPNetAddr>("127.0.0.1", 12345);
  ...
  client.connect([addr, &client]()
    ...
    client.writeMessage(message, [request](EasyTcp::AbstractProtocol::s_ptr msg_ptr) {
      DEBUGLOG("send message success, request[%s]", request.ShortDebugString().c_str());
    });


    client.readMessage("99998888", [](EasyTcp::AbstractProtocol::s_ptr msg_ptr) {
...
void test_rpc_channel()
{
  ...
  std::shared_ptr<EasyTcp::RpcClosure> closure = std::make_shared<EasyTcp::RpcClosure>(nullptr, [request, response, channel, controller]() mutable
                                                                                     {
  ...
  {
    std::shared_ptr<EasyTcp::RpcChannel> channel = std::make_shared<EasyTcp::RpcChannel>(EasyTcp::RpcChannel::FindAddr("127.0.0.1:12345"));
    ;
