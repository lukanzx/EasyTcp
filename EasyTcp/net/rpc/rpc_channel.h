...
#define NEWRPCCONTROLLER(var_name) \
  std::shared_ptr<EasyTcp::RpcController> var_name = std::make_shared<EasyTcp::RpcController>();

#define NEWRPCCHANNEL(addr, var_name) \
  std::shared_ptr<EasyTcp::RpcChannel> var_name = std::make_shared<EasyTcp::RpcChannel>(EasyTcp::RpcChannel::FindAddr(addr));
