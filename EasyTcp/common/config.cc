...
#define READ_XML_NODE(name, parent)                                         \
  ...
  if (!name##_node)                                                         \
  {                                                                         \
    printf("Start EasyTcp server error, failed to read node [%s]\n", #name); \
    exit(0);                                                                \
  }
...
#define READ_STR_FROM_XML_NODE(name, parent)                                     \
  ...
  {                                                                              \
    printf("Start EasyTcp server error, failed to read config file %s\n", #name); \
    exit(0);                                                                     \
  }                                                                              \
...
namespace EasyTcp
{
  ...
  Config::Config(const char *xmlfile)
  {
    ...
    if (!rt)
    {
      printf("Start EasyTcp server error, failed to read config file %s, error info[%s] \n", xmlfile, m_xml_document->ErrorDesc());
      exit(0);
    }
